...
Like for regression, we have several options for prediction. Note, there is a bug in the predict.forest function for binary responses.
```{r}
fit = forest(xtrain,ytrain,xtest,ytest,var.type="ustat",B=10)
names(fit)
u_train_oob = fit$predicted        # Case (1)
...
We can also estimate the variance of the train set predicted probabilities. Note: there is a bug here because probability > 1, need to look closer at how binary responses are being tracked.
```{r}
...
Our own implementation of the infinitesimal jackknife variance. Note, there is a bug in how binary responses are being tracked when sampling with replacement.
```{r}
fit = forest(xtrain,ytrain,var.type="infjack")
ij2_train_oob = fit$predicted   # Case (1)
ij2 = forest.varIJ(fit$predictedAll,fit)
